...
const generateEmailContent = (type: 'NEW' | 'STATUS_UPDATE', complaint: IComplaint) => {
    ...
    if (type === 'NEW') {
        ...
        html = `
            ...
            <p><strong>Priority:</strong> <span style="color: ${complaint.priority === 'High' ? 'red' : complaint.priority === 'Medium' ? 'orange' : 'green'};">${complaint.priority}</span></p>
...
export const sendComplaintEmail = async (type: 'NEW' | 'STATUS_UPDATE', complaint: IComplaint) => {
    if (!ADMIN_EMAIL || !EMAIL_USER) {
        console.error("Email environment variables (ADMIN_EMAIL or EMAIL_USER) are missing. Skipping email.")
        return
    }
